...
#'
#' Performs Global Distance Metric Learning (GDM) on the given data.
#' 
#' Put GDM function details here.
#' 
...
#' \item{diagonalA}{GDM suggested Mahalanobis matrix}
...
#'
#' # transform data using GDM 
#' result <- DiagA(data, simi, dism)
#' newData <- data %*% sqrt(result[[1]])
#'
#' # plot original data
...
#' # plot GDM transformed data
#' scatterplot3d(newData, color = color, cex.symbols = 0.6, 
#'			  xlim = range(data[, 1], newData[, 1]), 
#'			  ylim = range(data[, 2], newData[, 2]),
#'			  zlim = range(data[, 3], newData[, 3]))
			  
DiagA <- function(data, simi, dism, C0 = 1, S1 = NULL, D1 = NULL, fudge = 0.000001, threshold = 0.001, reduction = 2) {
		data <- as.matrix(data)
		simi <- as.matrix(simi)
		dism <- as.matrix(dism)
		N <- dim(data)[1]
		...
		return(list("diagonalA " = diag(as.numeric(a)), "error" = error))
}
